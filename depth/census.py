...
def depth_hazy_clear(hazy_image, clear_image, a, beta):
    dividend_r = -(np.log(hazy_image[:,:,0] - a[0]) - np.log(clear_image[:,:,0] - a[0])) / beta
    dividend_g = -(np.log(hazy_image[:,:,1] - a[1]) - np.log(clear_image[:,:,1] - a[1])) / beta
    dividend_b = -(np.log(hazy_image[:,:,2] - a[2]) - np.log(clear_image[:,:,2] - a[2])) / beta
...
image_dark_channel = get_dark_channel(H, 7)
...
transmission_rate = get_transmission(H, atmosphere_light, 0.95, 7)

# d(x)를 구한다. 산란 계수를 정해야 한다.
a = depth_hazy_clear(H, D, atmosphere_light, 0.4)
'''
...
depth = get_depth(transmission_rate, 0.4)
depth = np.concatenate((depth, get_depth(transmission_rate, 0.6)))
depth = np.concatenate((depth, get_depth(transmission_rate, 0.8)))
plt.imshow(depth, cmap="gray")
'''
plt.show()
'''
